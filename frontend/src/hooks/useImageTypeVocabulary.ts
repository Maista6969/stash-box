 ...
 *
 * Both come off one vocabulary query, so a component that wants both does not
 * ask twice. Apollo caches per set of variables, so every caller of this hook
 * shares one request -- but not with a caller passing a target or
 * include_disabled, which are different entries.
 *
