...
describe("ImageLightbox off-aspect badge", () => {
  const offFrame = () => screen.queryByText(/off frame/i);
  ...
  it("says nothing when the image matches its template", () => {
    setup({ a: FACE });
    expect(offFrame()).toBeNull();
  });

  it("flags an image whose proportions are not the template's", () => {
    renderForm(
      <ImageLightbox
        images={[{ ...img("a"), width: 300, height: 300 }]}
  ...
  // badging every correctly cropped image would make the badge worthless.
  it("tolerates rounding", () => {
    renderForm(
      <ImageLightbox
        images={[{ ...img("a"), width: 799, height: 1200 }]}
        cropTemplates={{ a: FACE }}
        onClose={() => {}}
      />,
    );
  ...
  // An SVG stores -1 for both dimensions. Badging it would say nothing true.
  it("says nothing about an image with no real dimensions", () => {
    renderForm(
      <ImageLightbox
        images={[{ ...img("a"), width: -1, height: -1 }]}
        cropTemplates={{ a: FACE }}
        onClose={() => {}}
      />,
    );
...
describe("ImageLightbox guides on an image that does not fit", () => {
  ...
  it("says so in the caption too", async () => {
    await show(square("a"));
    expect(screen.getByText(/off frame/)).toBeInTheDocument();
  });
  ...
  it("leaves a fitting image alone", async () => {
    ...
    expect(screen.queryByText(/off frame/)).toBeNull();
  });

  // The label margin is a fifth of the column. Reserving it for images that
  // will never draw a label -- scene images, edit diffs, anything with no
  // template -- shifts every lightbox in the app sideways for nothing.
  //
  // Reserving it only while the guides are drawn is the other way to get it
  // wrong, and is what shipped first: the picture then jumps sideways every
  // time the toggle is pressed. The room follows the template, not the toggle.
  describe("the room reserved for guide labels", () => {
    const guided = () =>
      document.querySelector(".ImageLightbox-main")?.className ?? "";

    it("is not taken when the image has no template", () => {
      setup();
      expect(guided()).not.toContain("ImageLightbox-main-guided");
    });

    it("is taken as soon as there is a template, before any toggling", () => {
      setup({ a: FACE });
      expect(guided()).toContain("ImageLightbox-main-guided");
    });

    it("does not move the picture when the guides are shown", async () => {
      const { user } = setup({ a: FACE });
      const before = guided();

      await user.click(screen.getByRole("button", { name: "Show guides" }));
      expect(guided()).toBe(before);
