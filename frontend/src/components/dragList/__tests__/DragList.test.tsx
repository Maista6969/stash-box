...
describe("DragList", () => {
  ...
  describe("dragging", () => {
    ...
    it("does not disturb an enclosing list", () => {
      ...
      expect(
        outerRows().map((row) =>
          row.querySelector(".DragList-handle")?.getAttribute("title"),
        ),
      ).toEqual([
        "Drag Alpha to reorder, or use the arrow keys",
        "Drag Bravo to reorder, or use the arrow keys",
        "Drag Charlie to reorder, or use the arrow keys",
      ]);
    });
