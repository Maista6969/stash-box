...
interface Props<T> {
  ...
  keyOf: (item: T) => string;
  /** Names the item in the handle's title. */
...
// A list reordered by dragging a grip handle, or with the arrow keys on it.
// Rows are draggable only while the pointer is over their handle, otherwise nested controls would be impossible to use
const DragList = <T,>({
  items,
  keyOf,
  labelOf,
    ...
    >
      {draft.map((item, index) => (
        ...
        >
          <button
            type="button"
            className={cx("minimal", `${CLASSNAME}-handle`)}
            title={`Drag ${labelOf(item)} to reorder, or use the arrow keys`}
