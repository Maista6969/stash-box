...
test("admin switches a type off and it stops being offered", async ({
  ...
  try {
    ...
    // The inner list only: the group's own row contains the type name too
    const toggle = adminPage
      .locator(".DragList .DragList .DragList-row", {
        has: adminPage.getByText(typeName, { exact: true }),
      })
      .getByRole("checkbox");
    await toggle.click();
    ...
    await expect(editPage.locator(".ImageLabels-option").first()).toBeVisible();
    await expect(
      editPage.locator(".ImageLabels-option", { hasText: typeName }),
    ).toHaveCount(0);
  } finally {
