...
// cropFallback cuts an upload down to a frame, rotating it first if asked
//
...
// developer on a vips-less platform able to run the thing.
//
// Compiled unconditionally -- only the Crop dispatch in crop_novips.go is
// build-tagged -- so the rotation math here runs under the same CI as
// everything else instead of living untested behind a tag nobody builds.
func cropFallback(data []byte, rect CropRect) ([]byte, error) {
