...
//
// Everything the file format dictates (like the signatures, the guides resource
...
// creation order, which is not sorted
...
func assertGuides(t *testing.T, got []croptemplate.Guide, want []croptemplate.Guide) {
	t.Helper()
	...
	for i := range want {
		...
		// A tolerance because positions are a division,
		// and the templates put thirds on a canvas that cannot hold them exactly
...
func TestParseReadsGuidesAsFractions(t *testing.T) {
	...
	// Sorted by axis, then position: the margins and quarters across,
	// then the hair, eye line, chin and bottom margin down
	assertGuides(t, template.Guides, []croptemplate.Guide{
		{Axis: croptemplate.AxisX, Position: 12.0 / 800},
		{Axis: croptemplate.AxisX, Position: 0.25},
		{Axis: croptemplate.AxisX, Position: 0.50},
		{Axis: croptemplate.AxisX, Position: 0.75},
		{Axis: croptemplate.AxisX, Position: 788.0 / 800},
		{Axis: croptemplate.AxisY, Position: 0.025},
		{Axis: croptemplate.AxisY, Position: 0.425},
		{Axis: croptemplate.AxisY, Position: 0.77},
		{Axis: croptemplate.AxisY, Position: 0.99},
	})
}
...
func TestParseMeasuresEachAxisAgainstItsOwnSpan(t *testing.T) {
	...
	assertGuides(t, template.Guides, []croptemplate.Guide{
		{Axis: croptemplate.AxisX, Position: 0.5},          // 400 of 800
		{Axis: croptemplate.AxisY, Position: 400.0 / 1200}, // the same pixel, a third of the way down
	})
}
