...
Drop the replacement in under the same name, regenerate the geometry, and
rebuild:

```
go generate ./internal/image/croptemplate
```

The server parses no PSD at runtime: it serves `templates_gen.go`, generated
from these files, and the binary embeds the files themselves. A replacement
dropped in without that step leaves the overlay on the old geometry --
`TestLoaderBytesParseToTheTemplateServed` re-parses every served file against
the table and fails when the two disagree.

Nothing in the code knows where a line is meant to sit, so there is nothing
else to update -- the tests check that whatever ships parses and is usable,
not that it matches a table someone typed.
