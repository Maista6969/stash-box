...
// CropRect is a frame to cut an image down to, as fractions of the image the client is looking at
//
...
type CropRect struct {
	...
	// Angle is degrees clockwise, applied before the frame is cut. X, Y, Width and Height
	// are measured against the _rotated_ image, which is larger than the original
	Angle float64
}

// ErrCropUnsupportedFormat reports an upload that cannot be cropped like SVG
