...
func Crop(data []byte, rect CropRect) ([]byte, error) {
	...
	if rect.Angle != 0 {
		// White rather than transparent: the corners a rotation exposes are going
		// to be cropped away in the ordinary case, and a JPEG has no alpha to put them in anyway
