...
// A gallery of three images differing only in state of dress, so that dimension alone decides the order
...
func (s *imageTypePreferenceTestRunner) testPreferenceReordersForOwnerOnly() {
	...
	// Dataloaders are built per request, so one viewer's ordering
	// cannot leak into another's even in the same process
	assert.Equal(s.t, instanceOrder, s.orderedFor(bystander, performerID),
...
func (s *imageTypePreferenceTestRunner) testPreferenceCannotOutrankGroups() {
	...
	// Even preferring face crops above everything, Shot type outranks Crop
	// and the preference cannot promote a detail shot past a portrait
...
// dimension, so a Crop preference could never have revealed which ordering was used
...
// A disabled dimension stops deciding anything
//
// The gallery in disagreeingGallery is ordered by Crop so
// switching Crop off should hand the decision to State of dress, the next enabled group
func (s *imageTypePreferenceTestRunner) testDisabledGroupDropsOutOfRanking() {
	...
	// One type off, its group still in use: Crop keeps deciding, but the image whose
	// only crop label was the disabled one now has nothing to be ranked on and falls behind.
	// Asserted separately from the group case because a disabled group would hide the type anyway:
	// this is the only way to see the type's own flag doing the work
	_, err = admin.resolver.Mutation().ImageTypeSetEnabled(admin.ctx, models.ImageTypeEnabledInput{
...
// Absent and empty are different answers for groups (absent leaves whatever the user had)
...
// order threw away a group order it had never mentioned: absent now means what it says
