...
//
// Served, not generated: these are the bytes the overlay in the edit form was parsed from,
// so the frame a contributor drags here and the one they drag in Photoshop are the same
func (rs cropTemplateRoutes) template(w http.ResponseWriter, r *http.Request) {
	...
	// which is right: this is a file to open in an editor, not to view
