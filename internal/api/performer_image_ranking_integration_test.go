...
func (s *performerRankingTestRunner) testUntypedPerformerUnchanged() {
	...
	// Exactly what the aspect-ratio comparator alone produces: closest to 2:3 first, widest last
	assert.Equal(s.t, []uuid.UUID{portrait.ID, nearPortrait.ID, wide.ID},
		s.orderedImages(performer.UUID()))
}

// Equally ranked images fall back to the comparator rather than to whatever order the database returned
...
// Organized breaks ties within a rank but never beats a better label rank
func (s *performerRankingTestRunner) testOrganizedBreaksTiesWithinARank() {
	reviewed, err := s.createTestImage(400, 600)
	assert.NoError(s.t, err)
	unreviewed, err := s.createTestImage(402, 600)
	assert.NoError(s.t, err)
	betterRanked, err := s.createTestImage(404, 600)
	assert.NoError(s.t, err)

	performer, err := s.createTestPerformer(&models.PerformerCreateInput{
		Name:     s.generatePerformerName(),
		ImageIds: []uuid.UUID{reviewed.ID, unreviewed.ID, betterRanked.ID},
	})
	assert.NoError(s.t, err)
	performerID := performer.UUID()

	// Same rank for the pair under test; a higher-priority label on the third
	s.assign(
		models.ImageTypeAssignment{ImageID: reviewed.ID, Type: models.ImageTypeEnumShotCandid},
		models.ImageTypeAssignment{ImageID: unreviewed.ID, Type: models.ImageTypeEnumShotCandid},
		models.ImageTypeAssignment{ImageID: betterRanked.ID, Type: models.ImageTypeEnumShotPortrait},
	)

	// The unreviewed twin gets the newer date, so organized has a date to beat
	date := "2024-01-01"
	...
	assert.Equal(s.t,
		[]uuid.UUID{betterRanked.ID, reviewed.ID, unreviewed.ID},
		s.orderedImages(performerID),
		"rank first, then organized over the newer date")
}

func TestOrganizedBreaksTiesWithinARank(t *testing.T) {
	s := createPerformerRankingTestRunner(t)
	s.testOrganizedBreaksTiesWithinARank()
}
