...
// createLabelledPerformer makes a performer carrying one uploaded image with the given labels
...
// wholesale, bypassing validation: intentionally uses combinations the real
// imageUpdate mutation would reject in order to isolate the dimension under test
...
func (s *performerImageTypeTestRunner) testUnlabelledImageIsStillTyped() {
	...
	// Untyped is a permanent, valid state: the image still appears, with no labels, rather than being omitted
...
// labels, since the same row could still be referenced elsewhere (however unlikely)
...
func (s *performerImageTypeTestRunner) testImageUpdateRejectsImpossibleCombination() {
	...
	rejected := []struct {
		...
		// nothing down to and including Torso can show either
		...
		// Can't see the face on the back of the head
...
// Switching a type off must not strand the images already carrying it
//
// Every save restates an image's labels, so refusing those outright would make the
// image unsaveable and the label unremovable, because the only way to drop it is a save
