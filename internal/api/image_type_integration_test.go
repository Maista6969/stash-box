...
func reversedOrderInput(groups []models.ImageTypeGroup) models.ImageTypeOrderInput {
	...
	for _, group := range slices.Backward(groups) {
		input.Groups = append(input.Groups, group.Key)
		for _, imageType := range slices.Backward(group.Types) {
			input.Types = append(input.Types, imageType.Key)
		}
	}
	return input
...
// restoreOrder puts the vocabulary back, so ordering tests do not leak into the rest of the suite
...
func (s *imageTypeTestRunner) testImageTypeGroups() {
	...
	for _, group := range groups {
		...
		for i, imageType := range group.Types {
			// The seed numbers each group's types densely from zero,
			// so position and sort_order agree only if both are right
...
func (s *imageTypeTestRunner) testImageTypeOrderUpdate() {
	...
	// rows share a sort_order, like DRESS taking 0 while SHOT still holds it.
	// That survives only because both unique constraints are deferred and the whole reorder commits once;
	// a statement-per-transaction implementation would abort here
...
func (s *imageTypeTestRunner) testConflictsAreServedBothWaysRound() {
	...
	// Seeded as ('CROP_FACE', 'DRESS_TOPLESS') and nothing else, so the reverse only exists if it was mirrored
...
// restoreEnabled switches the whole vocabulary back on,
// so an enabling test does not leak into the rest of the suite
