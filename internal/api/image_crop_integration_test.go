...
//
// Each call needs different pixels, because images are deduplicated by
// their hash and two identical uploads are one image by design
func (s *testRunner) uploadCropped(width, height int, crop *models.ImageCropInput) (*models.Image, error) {
	...
	for y := range height {
		for x := range width {
			// A gradient rather than a flat fill, so a crop of the wrong region
			// is visible as different pixels rather than looking identical to the right one
...
// of the stored bytes, so the same source and the same frame have to produce the same bytes
func TestIdenticalCropsDeduplicate(t *testing.T) {
	...
	// A different frame is a different image, or dedupe would be collapsing things it should not
