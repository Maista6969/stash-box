...
// cropUpload cuts an upload down to the frame a client asked for
func cropUpload(file []byte, input models.ImageCropInput) ([]byte, error) {
	...
	// Image doesn't need to change
	if rect.IsIdentity() {
		return file, nil
	}

	return imagepkg.Crop(file, rect)
}
