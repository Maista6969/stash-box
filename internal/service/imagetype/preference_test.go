...
// groupOrder and typeOrder read a vocabulary back as the two orderings it encodes
//
// The positions themselves are an implementation detail; what decides what a user sees
// is which dimension is compared before which, and which type wins inside one
...
// An unknown or repeated group is not a cosmetic problem the way an unknown or repeated type is
//
// Positions are handed out over the preference list, but the tuple is allocated to groupCount,
// so either one pushes a real group past the end of the tuple and Rank panics on an index out of range
// inside a GraphQL resolver, on a stale preference the user cannot see
//
// Asserted through Rank rather than through groupPosition, because that is where it would actually go wrong
...
// the instance ordering, so the two compose in either order:
// VocabularyFor applies groups first so this is what says it could apply them second and get the same vocabulary
func TestPreferencesComposeInEitherOrder(t *testing.T) {
	...
	// And that the composed result is actually both preferences, not one of them silently winning
