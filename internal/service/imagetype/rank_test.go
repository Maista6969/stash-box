...
// A two-group vocabulary in the shape the loader produces:
// CROP first, POSE second, each type at its own position within its group
//
...
func TestRank(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			// The order the assignment query happens to return must not change the answer.
			// FindImageTypesByImageIds orders by sort_order, so last-write-wins would have taken the lowest-priority type.
			// Two-from-one-group is not reachable through the API because every group is exclusive
			// but that is a property of the seed data and of the validators, not of the schema,
			// so the ranking has to behave when it happens anyway
		...
		{
			// A type the instance has switched off is missing from typeGroup entirely and simply stops counting
